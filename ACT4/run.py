...
def build_verilator_simulation_exe(ram_base: int):
    sv_files = list(HDL.rglob("*.sv")) + list(HDL.rglob("*.v"))
    ...
    cmd = f"verilator {TRACE_ARGS} --top {TOP_TB} -f rtl.files -cc --exe --build sim_main.cpp trace_dpi.cpp -Wno-UNOPTFLAT -GRAM_WORD_DEPTH={RAM_WORD_DEPTH} -GRAM_BASE_ADDR={ram_base}".split()
...
def run_test(
    ...
    SYMBOL_NAMES = ["rvtest_sig_begin", "rvtest_sig_end", "tohost", "fromhost"]
    RESULTS_DIR = elfs_dir.parent / "build"
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    WAVES_DIR = Path("./waves")
    WAVES_DIR.mkdir(parents=True, exist_ok=True)
    for elf in elfs_dir.rglob("*.elf"):
        # 跳过 Misalign 测试（你的核不支持不对齐访问）
        if "Misalign" in elf.name or "Misalign" in str(elf):
            continue
        # 不带扩展名的文件名
        stem = elf.stem
        ...
        cmd = [
            str(simulation_exe),
            f"+elf_name={stem}",
            f"+max_cycles={(symbols["rvtest_sig_end"]-entry_point)*max_cycles_factor//4}",
        ...
        cmd += [
            f"+sig_begin_canary={symbols['rvtest_sig_begin']:08x}",
            f"+sig_end_canary={symbols['rvtest_sig_end']:08x}",
        ]
